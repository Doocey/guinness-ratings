                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {reviews.map((review, index) => (
                      <tr key={review._id} className={`${index === 0 ? 'bg-[#FFD700] bg-opacity-50 hover:bg-opacity-90' : 'bg-slate-100'} hover:bg-slate-200`}>
                        ...
                        <td className="px-6 py-4 whitespace-pre-wrap">
                          <span className={`px-3 inline-flex text-xl py-2 font-semibold rounded-full ${index == 0 ? 'bg-green-200 text-green-800' : ''} ${index == 1 ? 'bg-slate-300 text-slate-800' : ''} ${index == 2 ? 'bg-amber-500 text-amber-800' : ''}`}>
                            {review.rating}
                          </span>
                        </td>
                      </tr>
